...
import {
  changeLocalResourceStatus,
  createLocalResource,
  getLocalResourceDetail,
  getLocalResourcePage,
  getLocalResourceStats,
  updateLocalResource
} from "./service";
...
const { tableData, pageable, searchParam, search, handleSizeChange, handleCurrentChange, getTableList } = useTable(
  async (params: Schedule.ResourceQuery) => {
    const result = await getLocalResourcePage(params);
...
const loadStats = async () => {
  const { data } = await getLocalResourceStats({
...
const openDetail = async (id: string) => {
  const { data } = await getLocalResourceDetail(id);
...
const handleStatusChange = async (id: string, enabled: boolean) => {
  const { data } = await changeLocalResourceStatus({
    id,
  ...
  if (currentDetail.value?.id === id) {
    currentDetail.value = (await getLocalResourceDetail(id)).data;
  }
